...
const LISTEN_CHANNELS: string[] = [
  ...
  'basecamp:timesheet-extra-people-result',
  IPC.CHECKIN_PROMPT,
  IPC.TODAY_CHANGED,
...
contextBridge.exposeInMainWorld('zenstate', {
  ...
  bcCreateTodo: (data: { projectId: number; todoListId: number; content: string; description?: string; parentId?: number; dueOn?: string }) => ipcRenderer.invoke(IPC.BC_CREATE_TODO, data),
  ...
  // v5.2 — Multi-person time-entry picker + check-ins + notifications.
  bcListProjectMembers: (projectId: number) => ipcRenderer.invoke(IPC.BC_LIST_PROJECT_MEMBERS, { projectId }),
  bcGetQuestionnaireForProject: (projectId: number) => ipcRenderer.invoke(IPC.BC_LIST_QUESTIONNAIRES, { projectId }),
  bcGetQuestions: (projectId: number, questionnaireId: number) => ipcRenderer.invoke(IPC.BC_GET_QUESTIONS, { projectId, questionnaireId }),
  ...
  // v5.7.0 — Folder organization for the Today plan.
  todayFolderCreate: (name: string) => ipcRenderer.invoke(IPC.TODAY_FOLDER_CREATE, { name }),
  todayFolderRename: (id: string, name: string) => ipcRenderer.invoke(IPC.TODAY_FOLDER_RENAME, { id, name }),
  todayFolderDelete: (id: string) => ipcRenderer.invoke(IPC.TODAY_FOLDER_DELETE, { id }),
  todayFolderReorder: (folderIds: string[]) => ipcRenderer.invoke(IPC.TODAY_FOLDER_REORDER, folderIds),
  todayFolderToggleCollapse: (id: string) => ipcRenderer.invoke(IPC.TODAY_FOLDER_TOGGLE_COLLAPSE, { id }),
  todayItemMove: (todoId: number, folderId: string | null, index: number) => ipcRenderer.invoke(IPC.TODAY_ITEM_MOVE, { todoId, folderId, index }),
  ...
  timerIdleRespond: (payload: { action: 'continue' | 'pause' | 'stop' | 'backdate'; stopAtIso?: string; enableMeetingMode?: boolean }) => ipcRenderer.send(IPC.TIMER_IDLE_RESPONSE, payload),
  timerSetMeetingMode: (on: boolean) => ipcRenderer.send(IPC.TIMER_SET_MEETING_MODE, on),
  timerTimesheetConfirm: (payload: { action: 'post' | 'discard'; hours?: string; notes?: string; durationSec?: number; additionalPersonIds?: number[] }) => ipcRenderer.send(IPC.TIMER_TIMESHEET_CONFIRM, payload),
